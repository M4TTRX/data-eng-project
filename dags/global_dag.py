...
GET_THERMAL_DATASET_URL = (
    "https://www.data.gouv.fr/api/1/datasets/63587afb1cc488641390f68e/"
)
GET_NUCLEAR_DATAET_URL = (
    "https://www.data.gouv.fr/api/1/datasets/63587afc1e8e90e9ce487174/"
)
...
def _import_thermal_clean_data():
    data_1 = pd.read_csv(
        "./dags/data/ingestion/thermal_plants_.csv",
    ...
    data_1 = data_1.rename(
        columns={
            "centrale": "plant",
            "point_gps_wsg84": "position",
            "commune": "city",
            "date_de_mise_en_service_industrielle": "start_date",
            "puissance_installee": "power (MW)",
        }
    )
...
def _import_nuclear_clean_data():
    data_1 = pd.read_csv(
        "./dags/data/ingestion/nuclear.csv", error_bad_lines=False, sep=";"
    )
    data_1 = data_1.drop(
        columns={
            "reserve_secondaire_maximale",
            "puissance_minimum_de_conception",
            "sub_sector",
            "perimetre_spatial",
            "combustible",
            "filiere",
    ...
    data_1 = data_1.rename(
        columns={
            "centrale": "plant",
            "sous_filiere": "sub_sector",
            "contrat_programme": "contract",
            "point_gps_wsg84": "position",
            "commune": "city",
            "date_de_mise_en_service_industrielle": "start_date",
            "puissance_installee": "power (MW)",
...
def _load_data_from_ingestion():
    ...
    imported_deaths = [
        ...
        for file_path in r.lrange("imported_death_files", 0, -1)
    ]
    ...
    files_to_load = [
        ...
        if file_path not in imported_deaths
    ]
    print(
        f"{len(imported_deaths)}/{len(death_files)} files already imported. Importing {len(files_to_load)} files: {str(files_to_load)}"
    )
    import json
    import hashlib
...
def _cleanse_death_data():
    ...
    death_data = [json.loads(element.decode("utf-8")) for element in result]
    import pandas as pd

    print("Successfully loaded death data")
    insee_code_to_geo = {
        loc["code_commune_INSEE"]: (loc["longitude"], loc["latitude"])
        for _, loc in pd.read_csv(
            f"{INGESTION_DATA_PATH}city_geo_loc.csv"
        ).iterrows()
    }
    query = ""
    count = 0
    for death in death_data:
        if death["location"] in insee_code_to_geo:
            ...
            if (
                math.isnan(location[0])
                or math.isnan(location[1])
                or birth_date is None
                or death_date is None
            ):
                continue
    ...
    with open(DEATH_INSERTION_QUERIES_PATH, "w") as f:
        f.write(query)
    ...
    for resource in thermal_resources["resources"]:
        if resource["format"] == "csv":
            ...
            if response.status_code == 200:
                with open(
                    f"{INGESTION_DATA_PATH}thermal_plants_.csv", "w"
                ) as outfile:
                    outfile.write(response.content.decode("utf-8"))
            else:
                print(f"Failed to get thermal plants resource")
...
def pull_all_death_files(max_resource=5):
    ...
    for resource in death_resources:
        ...
        if response.status_code == 200:
            with open(
                f'{INGESTION_DATA_PATH}death_{resource["title"]}', "w"
            ) as outfile:
                outfile.write(response.content.decode("utf-8"))
        else:
            print(
...
with TaskGroup(
    "ingestion_pipeline", "data ingestion step", dag=global_dag
) as ingestion_pipeline:
    start = DummyOperator(
        task_id="start",
        dag=global_dag,
    )
    ...
    get_city_code_geo = BashOperator(
        ...
        task_id="end", dag=global_dag, trigger_rule="all_success"
    )

    start >> [
        get_city_code_geo,
        get_nuclear_json,
        get_death_resource_list,
        get_thermal_json,
    ]
...
with TaskGroup(
    "staging_pipeline", "data staging step", dag=global_dag
) as staging_pipeline:
    start = DummyOperator(
        task_id="start",
        dag=global_dag,
    ...
    staging_end = DummyOperator(
        task_id="staging_end", dag=global_dag, trigger_rule="all_success"
    )
    ...
    start >> [
        import_nuclear_clean_data,
        import_thermal_clean_data,
        create_power_plants_table,
        create_death_table,
    ]
    (
        create_death_table
        >> load_data_from_ingestion
        >> cleanse_death_data
        >> death_emptiness_check
    )
    ...
    (
        ...
        >> import_thermal_clean_data
        >> create_power_plants_table
        >> create_plant_persist_sql_query
...
start_global = DummyOperator(
    task_id="start_global", dag=global_dag, trigger_rule="all_success"
)

end_global = DummyOperator(
    task_id="end_global", dag=global_dag, trigger_rule="all_success"
)
